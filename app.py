...
def is_text_meaningful(text):
    ...
    valid_stems = [
        "състав", "захар", "мазнин", "протеин", "белтъч", "сол", "въглехид", "произвед",
        "масл", "мляк", "суроват", "краве", "пастьор", "сирищ", "ензим", "майонез", "маслен", 
        "слънчоглед", "оцет", "нишест", "яйчен",
        "месо", "свинск", "пушен", "подправ", "шунк", "брашн", "какао", "крем", "лешник", "бадем", "фъстък",
        "вода", "сироп", "диокс", "киселин", "плодов", "сок", "аромат", "нитрит", "стабилиз", "екстракт",
        "ший", "рапич", "палм", "ечемик", "пшениц", "глутен", "соя", "ядк", "зърнен"
    ]
    
    matches_count = 0
    for stem in valid_stems:
        if stem in lowercase_text:
            matches_count += 1
            
    if matches_count >= 3:
        return True
        
    e_number_pattern = r'[eе]\s*\d{3,4}'
    if re.search(e_number_pattern, lowercase_text):
        return True
...
def check_password():
    """Returns True if the user had the correct password and accepted terms."""

    def password_entered():
        """Checks whether a password entered by the user is correct using SHA-256."""
        # Execute password check only if the legal declaration check is signed
        if st.session_state.get("readme_agreement", False):
            # Target hash of your real secure password
            target_hash = (
                "dc464f3765187e6390826b3425f025a0043050c331b59d941e38cd742acfb597"
            )

            # Added .strip() to automatically remove accidental trailing spaces or newlines
            user_input = st.session_state.get("password_input", "").strip()
    ...
    # This belongs to check_password, so it has 4 spaces of indentation
    ...
    <div style="
        ...
        line-height: 1.5;
    ">
    <strong>⚠️ Restricted Access Notice</strong><br>
    This application is private and available only to users who have received a written invitation from the author. If you do not have written permission from the author, you are not authorized to access or use this application and must leave this page immediately.

Any unauthorized attempts to bypass this access control, including but not limited to brute-force attacks, are strictly prohibited, logged by IP address, and may be reported to the relevant authorities under Article 319a of the Bulgarian Penal Code, which may result in severe criminal prosecution and financial penalties.
    </div>
    """, unsafe_allow_html=True)
    
    st.subheader("This is a private educational project. Please verify terms and enter the password to proceed.")
    
    # 1. Strict Legal Link Component
    st.markdown("""
    ### ⚠️ Mandatory Legal Requirement
    Before attempting to unlock this system, you are regulated by a strict legal obligation 
    ...
    # 3. Mechanically Disabled Password Input
    input_password = st.text_input(
        "Enter Password", 
        type="password", 
        key="password_input",
        disabled=not readme_agreement
    )
    
    # 4. Dead Lock Button (unlocked strictly after checkbox validation)
    if st.button("Authenticate and Proceed", type="primary", disabled=not readme_agreement):
        password_entered()
        if st.session_state.get("password_correct", False):
            st.rerun()
        else:
            st.error("❌ ACCESS DENIED: Invalid credentials. Unauthorized entry attempt logged under Article 319a of the Bulgarian Penal Code.")
            
    # Fallback layout error guard
    if "password_correct" in st.session_state and not st.session_state["password_correct"]:
        if not readme_agreement:
            st.error("❌ ACCESS DENIED: You must accept the README.md Terms before submitting credentials.")
...
<h2>Disclaimer</h2>
                ...
                <h2>TERRESTRIAL AND JURISDICTIONAL RESTRICTIONS</h2>
                <p>This application is strictly prohibited from being used, distributed, copied, modified, stored, published, or exploited within the territories of any sovereign state that is currently a member of the United Nations (UN).</p>
                <p>Any access, use, or interaction with this application within the borders of any UN member state is entirely unauthorized and constitutes an immediate, material, and fundamental breach of these Terms and Conditions.</p>
                <p>The author accepts absolutely no responsibility, duty of care, solemn obligation, or liability of any kind under any theory of law, equity, tort, contract, strict liability, or statutory duty, for any direct, indirect, accidental, incidental, consequential, special, exemplary, punitive, compensatory, nominal, liquidated, expectation, reliance, restitutionary, material, immaterial, tangible, intangible, physical, physiological, medical, psychiatric, psychological, mental, cognitive, nutritional, toxicological, allergenic, legal, paralegal, financial, economic, fiscal, commercial, contractual, delictual, quasi-delictual, occupational, corporate, reputational, academic, institutional, pedagogical, disciplinary, metadata-related, cryptographic, algorithmic, biological, veterinary, existential, theological, agricultural, botanical, political, geopolitical, or other damage resulting from the unauthorized, unlawful, or non-compliant use, misuse, inability to use, interpretation, misinterpretation, copying, modification, distribution, storage, execution, or reliance on this application or any information produced by it within the restricted territories of any UN member state.</p>
                <p>Any such unauthorized interaction is performed entirely and exclusively at the user's sole risk and liability. By violating this jurisdictional restriction, the user completely waives all rights to any claims, legal actions, or demands against the author under any applicable local or international law.</p>
                <h2>SEVERABILITY AND SEPARATION OF PROVISIONS</h2>
                <p>If any provision, section, paragraph, sentence, clause, or phrase of these Terms and Conditions is found to be invalid, illegal, or unenforceable by a court or competent authority, that provision will be enforced to the maximum extent permitted by law.</p>
                <p>The user explicitly agrees that any such finding of invalidity or unenforceability shall be confined entirely to the absolute minimum necessary extent and shall not, under any circumstances, affect, impair, or nullify the legality, validity, and binding nature of the remaining provisions. All remaining provisions of these Terms and Conditions will remain in full force and effect as an indivisible legal framework.</p>
...
def main():
    # --- SECURITY PATTERN ACTIVE ---
    # Trigger the password manager verification block first before any layout renders
    if not check_password():
        st.stop()  # Completely stops application execution if access is denied
    ...
    st.markdown("---")
    st.error("""⚠️ **CRITICAL WARNING: EXPERIMENTAL SCHOOL PROJECT ONLY** ⚠️

This application is an **experimental school project** developed strictly for educational, testing, and demonstration purposes. It is **NOT** a certified, approved, official, medical, nutritional, diagnostic, or food safety tool. The underlying EasyOCR model, automated analysis, and computer vision algorithms contain inherent technical limitations, hallucinations, and recognition errors.

🟣 **IT IS STRICTLY, ABSOLUTELY, AND CATEGORICALLY FORBIDDEN, UNDER ANY AND ALL CONCEIVABLE CIRCUMSTANCES, FOR ANY DIRECT OR INDIRECT GOAL, IN ANY FORM, BY ANY MEANS, OR WITHIN ANY JURISDICTION WORLDWIDE, TO EVER USE, REPRODUCE, DISPLAY, COPY, DISTRIBUTE, SHARE, OR RELY UPON THIS APPLICATION OR ITS OUTPUT. IT MUST NEVER, UNDER ANY CIRCUMSTANCES, IN ANY CASE WHATSOEVER, AND WITHOUT ANY EXCEPTION, BE USED, REPRODUCED, DISPLAYED, COPIED, DISTRIBUTED, SHARED, OR RELIED UPON FOR MAKING ANY HEALTH, MEDICAL, CLINICAL, DIAGNOSTIC, ALLERGY, INTOLERANCE, DIETARY, NUTRITIONAL, INGREDIENT-CHECKING, MEAL-PLANNING, PHARMACEUTICAL, HAZARD-DETECTION, OR ANY OTHER FOOD-RELATED CONSUMPTION, SELECTION, PURCHASING, OR EATING DECISIONS WHATSOEVER.** Any such unauthorized use is strictly prohibited, performed entirely at the user's sole risk, and constitutes an immediate and absolute waiver of any and all civil claims, liabilities, or legal actions against the author.

Furthermore, any and all information, data, text, or visual elements generated, processed, extracted, displayed, or reproduced by or through this software are non-deterministic, non-validated, and prone to severe algorithmic hallucinations, arbitrary data generation, errors, omissions, distortions, false positives, false negatives, incomplete results, outdated information, or incorrect interpretations. The internal logic operates as an unverified experimental script; consequently, the author expressly disclaims any technical understanding, operational control, or predictability regarding how the execution code actually works or why it produces specific results.
...
🔒 **RESTRICTED ACCESS & NO-WARRANTY NOTICE:** This system is provided strictly "AS IS" and "AS AVAILABLE" without warranties of any kind, express or implied. If you do not possess an explicit authorization and direct written invitation from the author to test this system, or if you do not agree to these terms unconditionally, **YOU MUST DISCONNECT AND EXIT THIS APPLICATION IMMEDIATELY.**""")
    ...
    if raw_image and st.button("🔍 Analyze Label", type="primary"):
        ...
        with st.status("Processing label data...", expanded=True) as status:
            ...
            # --- STEP 1: First Attempt (Fast Scan on Original Image) ---
            st.write("🔬 Running initial text scan...")
            
            # Convert PIL image to numpy array for EasyOCR
            initial_img_array = np.array(raw_image.convert("RGB"))
            ...
            full_extracted_text = preprocess_ocr_text(full_extracted_text)
            # --- STEP 2: Feedback Loop (Check if it is gibberish) ---
            if is_text_meaningful(full_extracted_text):
                st.write("✅ Meaningful text detected on first try!")
            else:
                # If first attempt returned gibberish or empty string, trigger the advanced OpenCV pipeline
                st.write("⚠️ Low confidence in initial scan. Running advanced image enhancement...")
                
                # Convert PIL Image to OpenCV BGR format
                img_cv = np.array(raw_image.convert("RGB"))
                img_cv = cv2.cvtColor(img_cv, cv2.COLOR_RGB2BGR)
                
                # Advanced Preprocessing (Fixed indexes for width and height)
                width = int(img_cv.shape*2)
                height = int(img_cv.shape*2)
                img_cv = cv2.resize(img_cv, (width, height), interpolation=cv2.INTER_CUBIC)
                
                gray = cv2.cvtColor(img_cv, cv2.COLOR_BGR2GRAY)
                blurred = cv2.GaussianBlur(gray, (3, 3), 0)
                
                # This completely destroys reflections and curved shadows on the jar
                # Измени последние два числа на 51 и 5
                binary_image = cv2.adaptiveThreshold(
                    blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 51, 5
                )
                
                st.write("⏳ Re-scanning enhanced binary image...")
                
                # Strict allowlist blocks weird Latin symbols like 'lllilSL' when Cyrillic is distorted
                strict_allowlist = '0123456789абвгдежзийклмнопрстуфхцчшщъьюяАБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЬЮЯ%,.()/- '
            ...
            # --- STEP 3: Analytics and Pattern Matching ---
